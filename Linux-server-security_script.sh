...
# - FIXED: AppArmor assessment now detects partially unloaded/teardown states instead of reporting false GREEN
# - IMPROVED: AppArmor enforce is skipped by default on Docker/Podman hosts unless explicitly forced
# - FIXED: Idempotence proof is skipped in step-by-step mode to avoid hidden replays of skipped sections
# - IMPROVED: SSH hardening now detects keyboard-interactive/2FA AuthenticationMethods and avoids disabling required methods
# - IMPROVED: SSH validation now prints the real sshd -t error output, and PermitRootLogin is normalized to prohibit-password in managed drop-ins
# ============================================================================
...
run_assessment() {
    ...
    if command -v aa-status >/dev/null 2>&1; then
        local enforced complain loaded aa_enabled_state loaded_profiles docker_only=false
        ...
        loaded=$(aa-status 2>/dev/null | grep "profiles are loaded" | grep -oP '^\d+' || echo "0")
...
' "$loaded_profiles" | sed '/^\s*$/d' | wc -l)
        ...
        if [[ "$aa_enabled_state" != "0" ]]; then
            ...
            record_check "APPARMOR" "PASS" "AppArmor: ${enforced:-0} enforce, ${complain:-0} complain"
        fi
    else
